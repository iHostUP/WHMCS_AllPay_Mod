...
function allpaybarcode_config() {
    $configarray = array(
	 ...
	 "InvoicePrefix" => array("FriendlyName" => "帳單前綴", "Type" => "text","Value" => "BAR","Description" => "必填", "Size" => "5", ),
...
function allpaybarcode_link($params) {
	...
	$post_value= array(
		...
		'ReturnURL' => $systemurl.'/modules/gateways/callback/allpaybarcode.php',
		...
		'PaymentInfoURL' => $systemurl.'/modules/gateways/allpay/allpaygetbarcode.php',
	);
	
	$CheckMacValue = curl_allpaybarcode_post($CheckMacValueurl,$post_value);
	...
	if($row==null){
		...
		<input type=hidden name="ReturnURL" value="'.$systemurl.'/modules/gateways/callback/allpaybarcode.php">				<!--付款成功後POST的地址-->
		...
		<input type=hidden name="PaymentInfoURL" value="'.$systemurl.'/modules/gateways/allpay/allpaygetbarcode.php">	<!--回傳給WHMCS取號通知-->
		<input type="submit" class="btn btn-success btn-sm" value="產生繳費條碼">
		</form>';
	}else{
		$code = '<p>帳單編號：'.$row[1].'</p>
		<p>繳費條碼1：<img src=https://pay-stage.allpay.com.tw/bank/tcbank/cnt/GenerateBarcode?barcode='.$row[8].'></img></p>
		<p>繳費條碼2：<img src=https://pay-stage.allpay.com.tw/bank/tcbank/cnt/GenerateBarcode?barcode='.$row[9].'></img></p>
		<p>繳費條碼3：<img src=https://pay-stage.allpay.com.tw/bank/tcbank/cnt/GenerateBarcode?barcode='.$row[10].'></img></p>
		<p>繳費期限：'.$row[11].'</p>
		<p>繳費金額：'.$row[4].'</p>';
	}
...
function curl_allpaybarcode_post($url,$post)
{
	 $ch = curl_init();
